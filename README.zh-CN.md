...
- **独立通知计划**：一个订阅可配置多个互不影响的通知计划，每个计划拥有独立的周期、金额、下次日期、通知时间和标题/正文模板；适合同时管理“每月付款”和“每三个月合租收款”。
...
打开订阅编辑窗口，在“独立通知计划”区域添加多个计划。以合租场景为例：

- “本月付款”：周期 `1 月`，填写每月付费日期和付款金额。
- “合租收款”：周期 `3 月`，填写下一次收款日期、应收金额和单独的收款模板。

标题和正文支持这些变量：`{{subscription.name}}`、`{{subscription.amountWithCurrency}}`、`{{plan.name}}`、`{{plan.amountWithCurrency}}`、`{{plan.nextDate}}`、`{{plan.intervalCount}}`、`{{plan.intervalUnit}}`、`{{plan.notes}}`。计划通知沿用系统已配置的通知渠道，并按“计划 + 日期”去重；启用“通知后自动顺延”后，发送成功会自动计算下一次日期。
