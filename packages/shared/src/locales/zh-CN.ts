export default {
  ...
  subscriptions: {
    ...
    form: {
      ...
      notificationPlansTitle: '独立通知计划',
      notificationPlansHint: '同一个订阅可以有多个独立周期，例如每月付款提醒、每三个月合租收款提醒。',
      addNotificationPlan: '添加计划',
      notificationPlanNumber: '通知计划 {number}',
      notificationPlanName: '计划名称',
      notificationPlanNamePlaceholder: '例如：本月付款 / 合租收款',
      notificationPlanNextDate: '下次通知日期',
      notificationPlanInterval: '重复周期',
      notificationPlanTime: '通知时间',
      notificationPlanEnabled: '启用',
      notificationPlanAutoAdvance: '通知后自动顺延',
      notificationPlanTitleTemplate: '通知标题模板',
      notificationPlanTitlePlaceholder: '例如：{{plan.name}}：{{subscription.name}}',
      notificationPlanBodyTemplate: '通知正文模板',
      notificationPlanBodyPlaceholder: '支持变量：{{subscription.name}}、{{plan.name}}、{{plan.amountWithCurrency}}、{{plan.nextDate}}、{{plan.notes}}',
      notificationPlanNotesPlaceholder: '计划备注（可选）',
      notificationPlanVariables: '可用变量：{{subscription.name}} {{subscription.amountWithCurrency}} {{plan.name}} {{plan.amountWithCurrency}} {{plan.nextDate}} {{plan.intervalCount}} {{plan.intervalUnit}} {{plan.notes}}',
      notificationPlansEmpty: '尚未添加独立通知计划',
      notificationPlanInvalid: '请补全通知计划名称、日期，并将通知时间填写为 HH:MM',
      logo: {
        upload: '点击上传',
        placeholder: 'Logo',
        panelTitle: '选择 Logo',
...
- 如果原文信息有限，就直接给出 1 到 2 句自然语言摘要。`
  ...
  scheduler: {
    ...
    logs: {
      ...
      notificationPlanScan: '[cron] 独立通知计划扫描：候选 {processedCount}，命中 {matchedPlanCount}，通知 {notificationCount}，成功渠道 {successful}',
